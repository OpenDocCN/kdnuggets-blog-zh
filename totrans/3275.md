...
检测车道线是自动驾驶车辆在道路上行驶的基本任务。这是进行其他路径规划和控制操作（如刹车和转向）的基础。让我们开始实现它们吧！
...
> 注：该项目源自 [Udacity 博客](https://github.com/udacity/CarND-LaneLines-P1)。所有生成的媒体使用了本文中解释的自定义 CV 配置。

**步骤 0: 介绍**

在处理视频之前，让我们先处理静态图像，因为静态图像更容易调试。这是我们将要处理的图像。
...
然后，我们对图像应用高斯平滑函数。这是为了使 Canny 边缘检测能够平滑图像中的异常梯度。
...
我们需要检测车道边缘，因为车道与周围道路表面之间的对比提供了检测车道线的有用信息。

Canny 边缘检测是一种利用图像像素值的水平和垂直梯度来检测边缘的算子。对该算法有更深入了解可以在[这里](http://docs.opencv.org/trunk/da/d22/tutorial_py_canny.html)找到。
...
*注意，边缘检测器捕捉到了所有车道线以及周围的边缘，如树木*

**步骤 3: 遮罩掉不在感兴趣区域中的点**

汽车摄像头的感兴趣区域仅包括其视野内的两条车道，而不包括其他任何多余的区域。我们可以通过创建一个多边形的感兴趣区域来过滤掉多余的像素，移除所有不在多边形中的像素。
...
现在我们已经在感兴趣区域检测到了边缘，我们想要识别表示车道线的直线。这时霍夫变换就派上用场了。

霍夫变换将“x 对 y”的直线转换为“梯度对截距”空间中的一个点。图像中的点将对应于霍夫空间中的直线。因此，霍夫空间中直线的交点将对应于笛卡尔空间中的一条直线。通过这种技术，我们可以从 Canny 边缘检测的像素输出中找到直线。有关霍夫变换的详细解释可以在这里找到。
...
+   [了解数据管理的 6 件事及其重要性…](https://www.kdnuggets.com/2022/05/6-things-need-know-data-management-matters-computer-vision.html)

+   [KDnuggets 新闻 2022 年 3 月 9 日：在 5 个步骤中构建机器学习网页应用](https://www.kdnuggets.com/2022/n10.html)
