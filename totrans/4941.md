...
车道线检测是自动驾驶汽车在道路上行驶时的一个基本任务。它是其他路径规划和控制动作（如刹车和转向）的基础。让我们开始实现这些功能吧！
...
> 注意：此项目源自 [Udacity 博客](https://github.com/udacity/CarND-LaneLines-P1)。所有生成的媒体使用了本文解释的自定义 CV 配置。

**步骤 0：介绍**

在处理视频之前，让我们先处理静态图像，因为静态图像更容易调试。这里是我们将要处理的图像。
...
然后，我们对图像应用高斯平滑函数。同样，这对于 Canny 边缘检测是必要的，以平滑图像中的异常梯度。
...
我们需要检测边缘以进行车道检测，因为车道与周围路面之间的对比为我们提供了检测车道线的有用信息。

Canny 边缘检测是一种运算符，利用图像像素值的水平和垂直梯度来检测边缘。可以在 [这里](http://docs.opencv.org/trunk/da/d22/tutorial_py_canny.html) 找到对该算法的更深入理解。
...
*注意边缘检测器捕捉到了所有车道线以及像树木这样的周围边缘*

**步骤 3：遮罩掉不在兴趣区域中的点**

汽车摄像头的兴趣区域仅包括其视野中的两个车道，而不包括任何其他区域。我们可以通过创建一个多边形兴趣区域来过滤掉这些额外的像素，并移除所有不在该多边形中的像素。
...
现在我们已经检测到了兴趣区域中的边缘，我们希望识别出指示车道线的直线。这就是霍夫变换发挥作用的地方。

霍夫变换将“x 对 y”直线转换为“梯度对截距”空间中的一个点。图像中的点将对应于霍夫空间中的直线。霍夫空间中直线的交点将对应于笛卡尔空间中的一条直线。利用这种技术，我们可以从 Canny 边缘检测输出的像素中找到直线。有关霍夫变换的详细解释可以在这里找到。
...
+   [你需要了解的 6 件数据管理相关的事以及为何重要……](https://www.kdnuggets.com/2022/05/6-things-need-know-data-management-matters-computer-vision.html)
