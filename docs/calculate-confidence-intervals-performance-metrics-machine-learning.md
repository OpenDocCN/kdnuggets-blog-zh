...
我们列出度量函数的列表，并调用`raw_metric_samples()`来获取仅针对 7 个样本数据集的结果。我们在这里调用`raw_metric_samples()`是为了理解——在使用下面的`ci_auto()`获取置信区间时并不是必须的，不过为`ci_auto()`指定一个度量列表（或仅一个度量）*是*必要的。
...
![](img/c939ced030c244587387abaa1149ead5.png)
