...
![数据科学中的降维技术](img/8b21115b2119e4c5418d96147df87680.png)
...
Isomap 通过建立邻域网络来启动。然后，它使用图距离来估计每对点之间的测地距离。最后，通过分解测地矩阵的特征值，将数据集嵌入到较低维度中。可以使用*Isomap()*类的*n_neighbours*超参数指定每个数据点要考虑的邻居数量。该类实现了 Isomap 算法。
